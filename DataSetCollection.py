...
from helper import argget, counter_generator, compile_arguments, generate_defaults_info
...
class DataSetCollection:
    ...
    _defaults = {
        ...
        'seed': {'help': 'Seed to be used for deterministic random sampling, given no threading is used', 'value': 1234},
    }

    def __init__(self, w, p, location, foldername, featurefiles, maskfiles, nclasses,  kw={}):
        ...
        self.tps = DataSetCollection.get_all_paths(join(location,foldername), self.featurefiles, self.maskfiles)
        
        # sampling and standardization properties
        ...
        self.padding_rule = 'constant'
        self.whiten = True
        self.whiten_subvolumes = False
        self.half_gaussian_clip = False
        self.presize_for_normalization = [None]
        self.regression = False
        self.lazy = True
        self.preloadall = False
        self.pyramid_sampling = False
    ...
    @staticmethod
    def get_all_paths(folder, featurefiles, maskfiles):
        '''
        computes list of all folders that are subfolders of folder and contain all needed featurefiles and maskfiles.
    ...
    def _get_features_and_masks(self, folder, featurefiles=None, maskfiles=None):
        ...
        features = [self.load(os.path.join(folder, i)).squeeze() for i in featurefiles]
        ...
        try:
            masks = [self.load(os.path.join(folder, i)) for i in maskfiles]
        except Exception:
...
class ThreadedDataSetCollection(DataSetCollection):
...
generate_defaults_info(DataSetCollection)
generate_defaults_info(ThreadedDataSetCollection)